      ...
      {status !== "connected" && (
        <div className="rounded-xl border border-stressed/40 bg-stressed/10 p-4 text-sm text-muted flex items-center justify-between gap-3">
          <div>
            ...
            <div>Check that the backend WebSocket is running at {wsUrl}.</div>
          </div>
          <button
            onClick={reconnect}
        ...
        <div className="rounded-xl border border-border bg-surface p-4">
          ...
          <div className="space-y-2 max-h-56 overflow-auto">
            ...
            {recentHistory.map((item) => {
              ...
              return (
                <div key={`${item.timestamp}-${item.level}-${item.score.toFixed(4)}`} className="flex items-center justify-between text-sm py-1 border-b border-border/40 last:border-0">
